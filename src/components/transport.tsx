...
import { updatePositionSlider, updateSongAction, updatePlayheadSeeking } from '../redux/actions';
...
export const Transport: React.FC<Props> = ({ }: Props) => {
  ...
  const sliderPositionPercentage = useSelector((state: AppState) => state.song.sliderPositionPercentage)
  ...
  useEffect(() => {
    if (refInput && refInput.current) {
      ...
      obs2$
        .subscribe(_ => { dispatch(updatePlayheadSeeking(true)); });
      obs3$
        .subscribe(_ => { dispatch(updatePlayheadSeeking(false)); });
    }
  }, [refInput.current]);
  ...
  return <div id="transport">
    ...
    <input ref={refInput} type="range" value={sliderPositionPercentage} onChange={(event) => {
